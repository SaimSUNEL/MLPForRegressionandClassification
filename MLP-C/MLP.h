...
{  Vector2D * temp ;
    ...
    for(int current_layer=1; current_layer < mlp_layer_count; current_layer++)
    {
        ...
        {
             temp = MatrixProductWithRowAdd(input, weight_array[current_layer-1], 1.0);
   ...
   // printf("\nmlp_layer_count : %d\n", mlp_layer_count);
    for( int current_layer = mlp_layer_count-2; current_layer>0;current_layer--)
    {
       // printf("\nCurrent layer = %d\n", current_layer);
        ...
        slice = VectorSlice2D(weight_array[current_layer+1], 1, weight_array[current_layer+1]->height, 0, weight_array[current_layer+1]->width);

        transpose = TransposeVector2D(slice);
        DestroyVector2D(slice);

        err = MatrixProduct(temp1, transpose);
       DestroyVector2D(transpose);
       DestroyVector2D(temp1);

        slice = VectorSlice2D(layer_results[current_layer], 0, layer_results[current_layer]->height, 1, layer_results[current_layer]->width);
        ...
        printf("\n\nSaim\n\n");
        */
        scalar_minus = ScalarMinusVector2D(1, slice);

        err = MatrixPairwiseProduct(MatrixPairwiseProduct(err, slice), scalar_minus);
       // printf("\n\nSUNEL\n\n");
       DestroyVector2D(slice);
    ...
    temp1 = err;
    slice = VectorSlice2D(weight_array[1], 1, weight_array[1]->height, 0, weight_array[1]->width);
    transpose = TransposeVector2D(slice);
    DestroyVector2D(slice);
     err = MatrixProduct(temp1, transpose);
     DestroyVector2D(transpose);
     ...
     Vector2D * sliced = VectorSlice2D(layer_results[0], 0, layer_results[0]->height, 1, layer_results[0]->width);
    scalar_minus = ScalarMinusVector2D(1, sliced);

     err = MatrixPairwiseProduct(MatrixPairwiseProduct(err, sliced), scalar_minus);//VectorSlice2D(layer_results[0], 0, layer_results[0]->height, 1, layer_results[0]->width));
     DestroyVector2D(scalar_minus);
     DestroyVector2D(sliced);
     ...
     DestroyVector2D(transpose);
     DestroyVector2D(err);
     //printf("\n\nLAyer 0 height : %d widht : %d\n\n", layer_updates[0]->height, layer_updates[0]->width);
