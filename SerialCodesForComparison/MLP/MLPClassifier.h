...
*/
void CreateMLP(float learning_rate_, int layer_count, ...)
{
...
void BackPropagate(Vector2D * input, Vector2D * labels)
{
    ...
    for( int current_layer = mlp_layer_count-2; current_layer>0;current_layer--)
    {
        ...
        if(current_layer == mlp_layer_count-2)
        {
          ...
          // DisplayVector2D(layer_updates[current_layer]);
    ...
    scalar_minus = ScalarMinusVector2D(1, layer_results[0]);
     ...
     //printf("\n\n layer 0 update \n");
    // DisplayVector2D(layer_updates[0]);
          //  exit(0);
    ...
    // we are updating the every weight by multiplying the change with the learning rate....
    for(int weight_index=0; weight_index < mlp_layer_count - 1;weight_index++)
    {
      //  printf("\n\nlayer update[%d]\n", weight_index);

      //  DisplayVector2D(layer_updates[weight_index]);
