...
Vector2D * w1, * w2, * b1, * b2, * w3, * b3;
Vector2D * output_1, * output_2, * output_3; 
Vector2D * bias_result_1, * bias_result_2, * bias_result_3;
...
#define FIRST_HIDDEN_LAYER_NODE_COUNT 96
#define SECOND_HIDDEN_LAYER_NODE_COUNT 32
...
void FeedForward(Vector2D * device_input, int batch_size)
{
	//input * w1
	dim3 block(32, 32);
	dim3 grid((FIRST_HIDDEN_LAYER_NODE_COUNT+block.x-1)/block.x, (batch_size+block.y-1)/block.y);
	...
	grid.x = (SECOND_HIDDEN_LAYER_NODE_COUNT+block.x-1)/block.x; grid.y = (batch_size+block.y-1)/block.y;
	...
	Sigmoid<<<grid, block>>>(output_2, output_2);
	cudaDeviceSynchronize();
	
	
	
	grid.x = (OUTPUT_NODE_COUNT+block.x-1)/block.x; grid.y = (batch_size+block.y-1)/block.y;
	MatrixProduct<<<grid, block>>>(output_3, output_2, w3);
	cudaDeviceSynchronize();
	
	//transpose ones * b2
	MatrixProduct<<<grid, block>>>(bias_result_3, ones_transpose, b3);
	cudaDeviceSynchronize();
	//bias2 + output of hidden layer * w2 - final output....  
	MatrixAdd<<<grid, block>>>(output_3, output_3, bias_result_3);
	cudaDeviceSynchronize();
	
	
	
	Exponential<<<grid, block>>>(output_3, output_3);
	cudaDeviceSynchronize();
	grid.x = 1; block.x = 1;
	Softmax<<<grid, block>>>(output_3, output_3);
	cudaDeviceSynchronize();
	
}

Vector2D * layer_2_error, * layer_1_error, * layer_3_error;
Vector2D * w1_update, * w2_update, * b1_update, * b2_update, * w3_update, * b3_update; 
Vector2D * output_1_transpose, * input_transpose, * output_2_transpose;
Vector2D * label_data;
Vector2D * device_whole_label;
Vector2D * device_whole_data;
Vector2D * device_input;
Vector2D * w2_transpose, * w3_transpose;
Vector2D * scalar_minus, * scalar_minus_2;
...
void BackPropagate(Vector2D * data, Vector2D * label, int batch_size)
{
	...
	MatrixSubtract<<<grid, block>>>(layer_3_error, label, output_3);
	cudaDeviceSynchronize();
	
	
	//output2 transpose
	dim3 grid2((SECOND_HIDDEN_LAYER_NODE_COUNT+block.x-1)/block.x, (batch_size+block.y-1)/block.y);
	TransposeVector2D<<<grid2, block>>>(output_2_transpose, output_2);
	cudaDeviceSynchronize();
	
	
	
	//W3 update...
	dim3 grid3((OUTPUT_NODE_COUNT+block.x-1)/block.x, (SECOND_HIDDEN_LAYER_NODE_COUNT+block.y-1)/block.y);
	MatrixProduct<<<grid3, block>>>(w3_update, output_2_transpose, layer_3_error);
	cudaDeviceSynchronize();
	...
	MatrixProduct<<<grid4, block>>>(b3_update, ones, layer_3_error);
	cudaDeviceSynchronize();
	...
	//W3 transpose
	dim3 grid5((OUTPUT_NODE_COUNT+block.x-1)/block.x, (SECOND_HIDDEN_LAYER_NODE_COUNT+block.y-1)/block.y);
	TransposeVector2D<<<grid5, block>>>(w3_transpose, w3);
	cudaDeviceSynchronize();	
	
	
	
	//Layer 2 error
	dim3 grid6((SECOND_HIDDEN_LAYER_NODE_COUNT+block.x-1)/block.x, (batch_size+block.y-1)/block.y);
	MatrixProduct<<<grid6, block>>>(layer_2_error, layer_3_error, w3_transpose);
	cudaDeviceSynchronize();
	
	
	MatrixPairwiseProduct<<<grid6, block>>>(layer_2_error, layer_2_error, output_2);
	cudaDeviceSynchronize();
	
	
	ScalarMinusVector2D<<<grid6, block>>>(scalar_minus_2, 1.0, output_2);
	cudaDeviceSynchronize();
	
	
	
	MatrixPairwiseProduct<<<grid6, block>>>(layer_2_error, layer_2_error, scalar_minus_2);
	cudaDeviceSynchronize();
	
	
	
	//output1 transpose
	dim3 grid22((FIRST_HIDDEN_LAYER_NODE_COUNT+block.x-1)/block.x, (batch_size+block.y-1)/block.y);
	TransposeVector2D<<<grid22, block>>>(output_1_transpose, output_1);
	cudaDeviceSynchronize();
	
	
	//W2 update...
	dim3 grid33((SECOND_HIDDEN_LAYER_NODE_COUNT+block.x-1)/block.x, (FIRST_HIDDEN_LAYER_NODE_COUNT+block.y-1)/block.y);
	MatrixProduct<<<grid33, block>>>(w2_update, output_1_transpose, layer_2_error);
	cudaDeviceSynchronize();
	
	
	
	//b2 update
	dim3 grid44((SECOND_HIDDEN_LAYER_NODE_COUNT+block.x-1)/block.x, (1+block.y-1)/block.y);
	MatrixProduct<<<grid44, block>>>(b2_update, ones, layer_2_error);
	cudaDeviceSynchronize();
	...
	//W2 transpose
	dim3 grid53((SECOND_HIDDEN_LAYER_NODE_COUNT+block.x-1)/block.x, (FIRST_HIDDEN_LAYER_NODE_COUNT+block.y-1)/block.y);
	TransposeVector2D<<<grid53, block>>>(w2_transpose, w2);
	cudaDeviceSynchronize();	
	
	
	
	//Layer 1 error
	dim3 grid62((FIRST_HIDDEN_LAYER_NODE_COUNT+block.x-1)/block.x, (batch_size+block.y-1)/block.y);
	MatrixProduct<<<grid62, block>>>(layer_1_error, layer_2_error, w2_transpose);
	cudaDeviceSynchronize();
	
	
	MatrixPairwiseProduct<<<grid62, block>>>(layer_1_error, layer_1_error, output_1);
	cudaDeviceSynchronize();
	
	
	ScalarMinusVector2D<<<grid62, block>>>(scalar_minus, 1.0, output_1);
	cudaDeviceSynchronize();
	
	
	
	MatrixPairwiseProduct<<<grid62, block>>>(layer_1_error, layer_1_error, scalar_minus);
	cudaDeviceSynchronize();
	...
	//w1 update....
	dim3 grid8((FIRST_HIDDEN_LAYER_NODE_COUNT+block.x-1)/block.x, (INPUT_NODE_COUNT+block.y-1)/block.y);
	MatrixProduct<<<grid8, block>>>(w1_update, input_transpose, layer_1_error);
	cudaDeviceSynchronize();
	
	//b1 update
	dim3 grid9((FIRST_HIDDEN_LAYER_NODE_COUNT+block.x-1)/block.x, (1+block.y-1)/block.y);
	...
	//w3_update * learning rate
	dim3 grid101((OUTPUT_NODE_COUNT+block.x-1)/block.x, (SECOND_HIDDEN_LAYER_NODE_COUNT+block.y-1)/block.y);
	ScalarMatrixProduct<<<grid101, block>>>(w3_update, learning_rate, w3_update);
	cudaDeviceSynchronize();
	
	//Apply w3 update
	MatrixAdd<<<grid101, block>>>(w3, w3, w3_update);
	cudaDeviceSynchronize();
	
	
	//b3_update * learning_rate
	dim3 grid111((OUTPUT_NODE_COUNT+block.x-1)/block.x, (1+block.y-1)/block.y);
	ScalarMatrixProduct<<<grid111, block>>>(b3_update, learning_rate, b3_update);
	cudaDeviceSynchronize();
	
	
	//Apply b3 update
	MatrixAdd<<<grid111, block>>>(b3, b3, b3_update);
	cudaDeviceSynchronize();
	...
	dim3 grid10((SECOND_HIDDEN_LAYER_NODE_COUNT+block.x-1)/block.x, (FIRST_HIDDEN_LAYER_NODE_COUNT+block.y-1)/block.y);
	...
	dim3 grid11((SECOND_HIDDEN_LAYER_NODE_COUNT+block.x-1)/block.x, (1+block.y-1)/block.y);
	...
	dim3 grid12((FIRST_HIDDEN_LAYER_NODE_COUNT+block.x-1)/block.x, (INPUT_NODE_COUNT+block.y-1)/block.y);
	...
	dim3 grid13((FIRST_HIDDEN_LAYER_NODE_COUNT+block.x-1)/block.x, (1+block.y-1)/block.y);
...
int main()
{
	...
	w1 = CreateWeightMatrix(INPUT_NODE_COUNT, FIRST_HIDDEN_LAYER_NODE_COUNT);
	b1 = CreateWeightMatrix(1, FIRST_HIDDEN_LAYER_NODE_COUNT);
	bias_result_1 = CreateVector2D(NULL, batch_size, FIRST_HIDDEN_LAYER_NODE_COUNT, false);
	
	output_1 = CreateVector2D(NULL, batch_size, FIRST_HIDDEN_LAYER_NODE_COUNT, false);
	output_1_transpose = CreateVector2D(NULL, FIRST_HIDDEN_LAYER_NODE_COUNT, batch_size, false);
	
	w1_update = CreateVector2D(NULL, INPUT_NODE_COUNT, FIRST_HIDDEN_LAYER_NODE_COUNT, false);
	b1_update = CreateVector2D(NULL, 1, FIRST_HIDDEN_LAYER_NODE_COUNT, false);
	
	//output 10 nodes....
	
	w2 = CreateWeightMatrix(FIRST_HIDDEN_LAYER_NODE_COUNT, SECOND_HIDDEN_LAYER_NODE_COUNT);
	w2_transpose = CreateVector2D(NULL, SECOND_HIDDEN_LAYER_NODE_COUNT, FIRST_HIDDEN_LAYER_NODE_COUNT, false);
	b2 = CreateWeightMatrix(1, SECOND_HIDDEN_LAYER_NODE_COUNT);
	bias_result_2 = CreateVector2D(NULL, batch_size, SECOND_HIDDEN_LAYER_NODE_COUNT, false);
	
	
	output_2 = CreateVector2D(NULL, batch_size, SECOND_HIDDEN_LAYER_NODE_COUNT, false);
	w2_update = CreateVector2D(NULL, FIRST_HIDDEN_LAYER_NODE_COUNT, SECOND_HIDDEN_LAYER_NODE_COUNT, false);
	b2_update = CreateVector2D(NULL, 1, SECOND_HIDDEN_LAYER_NODE_COUNT, false);
	
	w3 = CreateWeightMatrix(SECOND_HIDDEN_LAYER_NODE_COUNT, OUTPUT_NODE_COUNT);
	b3 = CreateWeightMatrix(1, OUTPUT_NODE_COUNT);
	
	w3_transpose = CreateVector2D(NULL, OUTPUT_NODE_COUNT, SECOND_HIDDEN_LAYER_NODE_COUNT, false);
	
	w3_update = CreateVector2D(NULL, SECOND_HIDDEN_LAYER_NODE_COUNT, OUTPUT_NODE_COUNT, false);
	b3_update = CreateVector2D(NULL, 1, OUTPUT_NODE_COUNT, false);
	output_3 = CreateVector2D(NULL, batch_size, OUTPUT_NODE_COUNT, false);
	bias_result_3 = CreateVector2D(NULL, batch_size, OUTPUT_NODE_COUNT, false);
	output_2_transpose = CreateVector2D(NULL, SECOND_HIDDEN_LAYER_NODE_COUNT, batch_size, false);
	
	layer_2_error = CreateVector2D(NULL, batch_size, SECOND_HIDDEN_LAYER_NODE_COUNT, false);
	layer_1_error = CreateVector2D(NULL , batch_size, FIRST_HIDDEN_LAYER_NODE_COUNT, false);
	layer_3_error = CreateVector2D(NULL, batch_size, OUTPUT_NODE_COUNT, false);
	
	
	scalar_minus  = CreateVector2D(NULL, batch_size, FIRST_HIDDEN_LAYER_NODE_COUNT, false);
	scalar_minus_2 = CreateVector2D(NULL, batch_size, SECOND_HIDDEN_LAYER_NODE_COUNT, false);
    ...
    float * train_data = (float *)malloc(sizeof(float)*train_count*784);
    for(int h = 0; h < train_count; h++)
    {
        for(int w=0; w < 784; w++)
        {
        //if(h == 0)printf("%f ", train_image[h][w]);
            train_data[h*784+w] = train_image[h][w];
        }
        
    }

    float * train_label_float = (float *)malloc(sizeof(float)*train_count);
     for(int h = 0; h < train_count; h++)
    {	
    ...
    float * test_data = (float *)malloc(sizeof(float)*test_count*784);
     for(int h = 0; h < test_count; h++)
    {
     ...
     float * test_label_float = (float *)malloc(sizeof(float)*test_count);
     for(int h = 0; h < test_count; h++)
    {
            test_label_float[h] = test_label[h];
    }

    Vector2D * data_set = CreateVector2DCPU(train_data, train_count, SIZE);

    Vector2D * image_labels = CreateVector2DCPU(train_label_float, train_count, 1 );
    ...
    Vector2D * test_image_fin = CreateVector2DCPU(test_data, test_count, SIZE);

    Vector2D * test_label_indices = CreateVector2DCPU(test_label_float, test_count, 1);
	...
	device_whole_data = CreateVector2D(data_set->data, data_set->height, 784);
	device_whole_label = CreateVector2D(one_hot_labels->data, one_hot_labels->height, 10);
	...
	PointerSet<<<1 ,1>>>(batch_data, device_whole_data, 0, batch_size);
	cudaDeviceSynchronize();
	
	PointerSet<<<1 ,1>>>(batch_label, device_whole_label, 0, batch_size);
	cudaDeviceSynchronize();
	...
	{
		toplam = 0;
	for(int batch_index = 0; batch_index < train_count/batch_size; batch_index++)
	{
		
		PointerSet<<<1 ,1>>>(batch_data, device_whole_data, (batch_index)*batch_size, batch_size);
	cudaDeviceSynchronize();
	
	PointerSet<<<1 ,1>>>(batch_label, device_whole_label, (batch_index)*batch_size, batch_size);
	cudaDeviceSynchronize();
	...
	Log2D<<<gridd, block>>>(output_3, output_3);
	cudaDeviceSynchronize();
	
	MatrixPairwiseProduct<<<gridd, block>>>(layer_3_error, batch_label, output_3);
	cudaDeviceSynchronize();
	dim3 b(1, 32);
	Sum2D<<<1, b>>>(layer_3_error);
	cudaDeviceSynchronize();
