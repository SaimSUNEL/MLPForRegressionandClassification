...
int main()
{
	...
	//first hidden layer 160 input 32754 
	...
	{
		toplam = 0;
	for(int batch_index = 1; batch_index < data_set->height/batch_size -1 ; batch_index++)
	{
	...
	MatrixSubtract<<<gridd, block>>>(layer_2_error, batch_label, output_2);
	
	MatrixPairwiseProduct<<<gridd, block>>>(layer_2_error, layer_2_error, layer_2_error);
	cudaDeviceSynchronize();
	dim3 k(1, 32);
