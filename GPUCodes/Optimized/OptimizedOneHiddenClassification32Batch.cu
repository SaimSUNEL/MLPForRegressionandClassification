...
int main()
{
	...
	*/
	...
	for(int iteration = 0 ; iteration < new_iteration_count;iteration++)
	{
		toplam = 0;
	...
	int temp;
		...
		for(int batch_index = 1; batch_index < 51;batch_index++)//data_set->height/batch_size; batch_index++)//10;batch_index++ );//data_set->height/batch_size; batch_index++)
		{
	...
	printf("\nNew iteration count : \n");
	scanf("%d", &new_iteration_count);
	if(new_iteration_count == 0);
	else
