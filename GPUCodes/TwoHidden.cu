...
#define SECOND_HIDDEN_LAYER_NODE_COUNT 64
...
int main()
{
	...
	#define ITERATION_COUNT 80
	for(int iteration = 0 ; iteration < ITERATION_COUNT;iteration++)
	{
		toplam = 0;
	for(int batch_index = 1; batch_index < train_count/batch_size; batch_index++)
	{
	...
	dim3 gride((OUTPUT_NODE_COUNT+block.x-1)/block.x, (batch_size+block.y-1)/block.y);
	MatrixSubtract<<<gride, block>>>(layer_3_error, batch_label, output_3);
	cudaDeviceSynchronize();
	...
	MatrixPairwiseProduct<<<gridd, block>>>(layer_3_error, layer_3_error, layer_3_error);
	cudaDeviceSynchronize();
	dim3 k(1, 32);
	Sum2D<<<1, k>>>(layer_3_error);
	cudaDeviceSynchronize();
