...
__device__ double error_sum[BATCH_SIZE];
...
program_start = clock();
	...
	device_whole_label = CreateVector2D(one_hot_labels->data, data_set->height, 10);
	...
	device_whole_test_data = CreateVector2D(test_samples->data, test_samples->height, 784);
	device_whole_test_label_data = CreateVector2D(test_labels->data, test_labels->height, 10);
	...
	for(int iteration = 0 ; iteration < ITERATION_COUNT;iteration++)
	{
		...
		*/
		...
		for(int batch_index = 1; batch_index < data_set->height/batch_size; batch_index++)
		{
			...
			cudaDeviceSynchronize();
			cudaMemcpyFromSymbol( &error_val, error_sum, sizeof(double)*BATCH_SIZE);
			//printf("\n\nIteration %d - Error : %f\n", iteration, error_val);	
			cudaDeviceSynchronize();
