...
#define BLOCK_X 32
#define BLOCK_Y 32
float learning_rate = 1.0e-6;

#define INPUT_NODE_COUNT 784
#define HIDDEN_LAYER_NODE_COUNT 96
...
program_start = clock();
    ...
    Vector2D * one_hot_labels = CreateOneHot(image_labels, OUTPUT_NODE_COUNT);


    Vector2D * test_samples = CreateVector2DCPU(test_data, test_count, SIZE);

    Vector2D * test_label_indices = CreateVector2DCPU(test_label_float, test_count, 1);
    Vector2D * test_labels = CreateOneHot(test_label_indices, OUTPUT_NODE_COUNT);
	...
	device_whole_data = CreateVector2D(data_set->data, data_set->height, INPUT_NODE_COUNT);
	...
	device_whole_test_data = CreateVector2D(test_samples->data, test_samples->height, INPUT_NODE_COUNT);
	device_whole_test_label_data = CreateVector2D(test_labels->data, test_labels->height, OUTPUT_NODE_COUNT);
	
	
	
	Vector2D * batch_test_data, * batch_label_data;
	batch_test_data = CreateVector2D(NULL, batch_size, test_samples->width, false);
	batch_label_data = CreateVector2D(NULL, batch_size, test_labels->width, false);
	...
	for(int iteration = 0 ; iteration < ITERATION_COUNT;iteration++)
	{
		...
		*/
		...
		for(int batch_index = 1; batch_index < data_set->height/batch_size; batch_index++)
		{
			...
			printf("\nITeration %d batch %d error %f \n", iteration, batch_index, toplam);
			
			
			
	
	printf("\n\nTest Accuracy : %f", (float(correct_number)/(correct_number+false_number)*100.0));
		...
		correct_number = 0 ;
			false_number = 0;
		for(int batch_index = 0; batch_index < test_labels->height/batch_size; batch_index++)
	{
	...
	for(int i = 0; i < BATCH_SIZE;i++)
	{
		...
		if( abs(predicted_labels[i] - test_label_indices->data[i + BATCH_SIZE*batch_index*test_label_indices->width]) < 0.1)
		{
